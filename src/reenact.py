...
from util.util import read_target, load_coeffs, load_landmarks
import cv2
...
def parse_args():
    ...
    parser.add_argument(
        "--config", type=str, required=True, help=".ini config file name"
    )
    parser.add_argument(
        "--src_img",
        type=str,
        required=True,
        help="input source actor image (.jpg, .bmp, .jpeg, .png)",
    )
    parser.add_argument(
        "--src_headpose",
        type=str,
...
def load_data(opt, headpose_file, landmark_file, img_file=None, load_img=True):
    ...
    face["landmarks"] = torch.from_numpy(
        np.array(load_landmarks(landmark_file))
    ).float()
    return face
...
if __name__ == "__main__":
    ...
    # off-line stage
    src_face["landmark_img"] = landmark_img_generator.generate_landmark_img(
        src_face["landmarks"]
    )
    src_face["landmark_img"] = [
        value.unsqueeze(0) for value in src_face["landmark_img"]
    ]
    model.set_source_face(
        src_face["img"].unsqueeze(0), src_face["headpose"].unsqueeze(0)
    )

    # on-line stage
    drv_face["landmark_img"] = landmark_img_generator.generate_landmark_img(
        drv_face["landmarks"]
    )
    drv_face["landmark_img"] = [
        value.unsqueeze(0) for value in drv_face["landmark_img"]
    ]
    model.reenactment(
        src_face["landmark_img"],
        drv_face["headpose"].unsqueeze(0),
        drv_face["landmark_img"],
    )
